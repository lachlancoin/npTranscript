...
#Set plot reactives
   ...
   gather_input_activity <- reactive( {
	list('options1' = input$options1,
 ...
 gather_input_abundance <- reactive( {
  list(
...
'angle' = input$angle,
'maxtrans' = input$maxtrans,
'conf.int' = input$conf.int,
'toplot5' = input$toplot5,
'toplot7' = input$toplot7,
'toplot8' = input$toplot8,
'tojoin' = input$tojoin
)
  ...
  gather_input_depth <- reactive( {
  list(  'options3' = input$options3,
  'textsize' = input$textsize,
  'conf.int' = input$conf.int,
  'depth_thresh' = input$depth_thresh,
  'group_by' = input$group_by,
  'motif' = input$motif,
  'maxKmers' = input$maxKmers,
  'test' = input$test,
  'waterfallKmer' = input$waterfallKmer,
  'waterfallOffset' = input$waterfallOffset,
  'loess' = input$loess,
  'toplot5' = input$toplot5,
  'tojoin' = input$tojoin,
  'toplot7' = input$toplot7,
  ...
  'maxtrans' = input$maxtrans,
   'molecules'=input$molecules,
          'cells'=input$cells,
          'times' = input$times,
	'min_x' = input$min_x, 'max_x' = input$max_x, 'alpha' = input$alpha
 ...
 #plotting functions
   ...
   depthPlot= function(params, plot_type, reuse=F) {
 ...
 if(is.null(session$userData$h5file)) return(ggplot())
   if(!file.exists(session$userData$h5file)) return(ggplot())
    ...
    showDepth  = "show_depth" %in% params$options3
    logy = "logy" %in% params$options3
    textsize=params$textsize
    ci=0
    if(  "showCI" %in% params$options3){
      ci=params$conf.int
    }
      zoom = "zoom" %in% params$options3
    
    depth_thresh = params$depth_thresh
    group_by=params$group_by
    showLegend = "showLegend" %in% params$options3
    reverseOrder=F
    merge_by="" #params$merge_by
  #  plot_type=params$depth_plot_type
    motif = isolate(params$motif)
    fastaseq =session$userData$fastaseq[[1]]
    fasta=session$userData$fasta[[1]]
    maxKmers=isolate(params$maxKmers)
    showORFs="showORFs" %in% params$options3
    fisher = params$test =="fisher"
    showErrors = "showErrors" %in% params$options3
    plotCorr = "plotCorr" %in% params$options3
    showSequence="showSequence" %in% params$options3
    if(plot_type=="depth") showWaterfall=F else showWaterfall="showWaterfall" %in% params$options3
    waterfallKmer=isolate(params$waterfallKmer)
   waterfallOffset=isolate(params$waterfallOffset)
    
    motifpos=list()
    if(nchar(motif>0) && !is.null(fastaseq)){
      motifpos= lapply(strsplit(motif,"\\|")[[1]], function(x) gregexpr(x,fastaseq, ignore.case=T)[[1]])
    }
    mergeCounts='mergeCounts' %in% params$options3
    showPeptides="showPeptides" %in% params$options3
    downsample="downsample" %in% params$options3
    tpm = "TPM_amongst_viral" %in% params$options3
  ...
  #  span = 0
    ...
    if(showDepth  && !is.null(h5file)){
      if(file.exists(h5file)){
        toplot = unique(c(isolate(params$toplot5)))#,isolate(params$toplot6))#,isolate(params$toplot7),isolate(params$toplot8))
        tojoin=isolate(params$tojoin)
        merge=F
        ...
        if(length(toplot)==0){
          ...
          toplot=c(isolate(params$toplot7),isolate(params$toplot8))
        ...
        if(length(toplot)>0 && nchar(toplot[1])>2 ){
          if("sumDepth" %in% params$options3) sumAll=TRUE
          mergeGroups=NULL
          ...
          }else if(length(toplot)>params$maxtrans){
            ##only show top number if not merging
            isoInfo=session$userData$isoInfo
            inds_k = sort(match(toplot,isoInfo$orfs$ORFs))
          #  print(inds_k)
         #   print(isoInfo$orfs$ORFs[inds_k][1:params$maxtrans])
            toplot = isoInfo$orfs$ORFs[inds_k][1:params$maxtrans]
          }
          molecules=params$molecules
          cells=params$cells
          times = params$times
        xlim =   c(isolate(params$min_x), isolate(params$max_x))
        alpha = isolate(params$alpha)
        linesize=0.001  #isolate(params$linesize)
  ...
  # depthPlot= function(params, plot_type, reuse=F, toreplace1, toreplace2) {

 # reuseData=F
 # #early return if no h5file
 # if(is.null(params$h5file)) return(ggplot())
   # if(!file.exists(params$h5file)) return(ggplot())
 
   # #boolean toggle plot on/off
    # showDepth  = "show_depth" %in% params$options3
	# #boolean toggle logy on/off
    # logy = "logy" %in% params$options3
	# #set textsize
    # textsize=params$textsize
	
	# #set confidence interval to 0 unless declared
    # ci=0
    # if(  "showCI" %in% params$options3){
      # ci=params$conf.int
    # }
      # zoom = "zoom" %in% params$options3
    # #set threshold for including transcripts
    # depth_thresh = params$depth_thresh
	# #set paramater to group transccripts
    # group_by=params$group_by
	# #boolean toggle legend on/off
    # showLegend = "showLegend" %in% params$options3
	# #??
    # reverseOrder=F
  ...
  # #  plot_type=input$depth_plot_type
    ...
    # motif = isolate(params$motif)
    # fastaseq =params$fastaseq[[1]]
    # fasta=params$fasta[[1]]
    # maxKmers=isolate(params$maxKmers)
    # showORFs="showORFs" %in% params$options3
    # fisher = params$test =="fisher"
    # showErrors = "showErrors" %in% params$options3
      ...
      # if(file.exists(h5file)){
       ...
       # depthOpts = 
         # list(total_reads=total_reads, toplot=toplot, downsample = downsample, span = span, 
              ...
              # ci = ci, depth_thresh = depth_thresh,
              # showWaterfall=showWaterfall,waterfallKmer=waterfallKmer,waterfallOffset=waterfallOffset, top10=maxKmers)
       # depthOpts_o = params$depthOpts[[which(names(params$depthOpts)==plot_type)]]
     # #  plotOpts_o = params$plotOpts[[which(names(params$plotOpts)==plot_type)]]
       
       # if(reuse ){
         # ggp = params$dataPlot[[which(names(params$dataDepth)==plot_type)]]
       # }else{
            ...
            # }else{
              # reuseData = F 
            # }
            # session$userData$depthOpts[[which(names(params$depthOpts)==plot_type)]] = depthOpts
...
#	p_plot = list(textsize=20, logy=T, showCI=F,riboon=F,angle=25,facet="none")
  ...
  p_plot = list()
	...
	p_plot$textsize=params$textsize
  p_data$molecules <-  params$molecules 
  p_data$useReadCount<- "useReadCount" %in% params$options2
  p_data$cells <- params$cells 
  p_data$times<-params$times
  p_data$splitby="off" #params$splitby
  splitby_vec=NULL
  p_data$xy=FALSE
    ...
    p_plot$facet=params$facet
    p_plot$showLegend = "showLegend" %in% params$options2
    p_plot$logy = "logy" %in% params$options2
    p_plot$showCI = "showCI" %in% params$options2
    p_plot$ribbon="ribbonCI" %in% params$options2
    p_data$showTPM="TPM_amongst_viral" %in% params$options2 || "TPM_amongst_all" %in% params$options2
    p_plot$showCI = "showCI" %in% params$options2
    p_data$merge='mergeCounts' %in% params$options2
    p_data$barchart="barchart" %in% params$options2
    p_data$reverseOrder="reverseOrder" %in% params$options2
    p_data$stack = "stacked" %in% params$options2
    p_data$calcTPMFromAll = "TPM_amongst_all"  %in% params$options2
    p_data$group_by=params$group_by
    p_plot$angle = params$angle
    p_data$merge_by=""  #params$merge_by
    p_data$max_trans = params$maxtrans
    p_data$conf.int=params$conf.int
    p_data$method="logit";
    useReadCount  = p_data$useReadCount;
    p_data$toplot = c(isolate(unique(params$toplot5)))#,isolate(params$toplot6))#,isolate(params$toplot7),isolate(params$toplot8))
    p_data$toplot = p_data$toplot[p_data$toplot!="-"]
    p_data$usegrep=F
    if(length(p_data$toplot)==0){
      p_data$toplot=c(isolate(params$toplot7),isolate(params$toplot8))
      p_data$toplot = p_data$toplot[unlist(lapply(p_data$toplot,nchar))>2]
      p_data$usegrep=T
    }
    p_data$tojoin=isolate(params$tojoin)
   ...
   # transcriptPlot<-function(params, toreplace1, toreplace2){
    # if(is.null(params$datafile)) return(ggplot())
    # if(!file.exists(params$datafile)) return(ggplot())
    # if(!"showTranscriptPlot" %in% params$options2) return(ggplot())
...
# #	p_plot = list(textsize=20, logy=T, showCI=F,riboon=F,angle=25,facet="none")
  ...
  # p_data$xy=FALSE
    ...
    # if(length(p_data$toplot)==0 || is.null(params$datafile ) ){
      # return(ggplot())
    # }
    ...
    # if(!is.null(prev_params)){
      ...
      # }
    # }
    # params$prev_params = list(p_plot=p_plot, p_data =p_data)
    # sec_axis_name = "Proportion (%)"
    # if(reuseData && !is.null(session$userData$results)){
      # print("reusing tpm data")
      # results_ = session$userData$results
    ...
    # }else{
      ...
      # countsTotal = params$countsTotal
      # countsHostVirus1 = NULL
      # subs = .extractTPM(datafile ,  total_reads,countsTotal, p_data, toreplace1 = toreplace1, toreplace2 = toreplace2)
  ...
  # }
    ...
    infectivityPlot <- function(params){
    if(!"showInfectivity" %in% params$options1) return(ggplot())
    currdir = session$userData$currdir
    barchart="barchart" %in% params$options1
    reverseOrder="reverseOrder" %in% params$options1
    showSecondAxis="showSecondAxis" %in% params$options1
    textsize=params$textsize
    conf.int=params$conf.int
    facet = params$facet1
    showLegend = "showLegend" %in% params$options1
    countsTotal=session$userData$countsTotal
    ...
    if(file.exists(infilesAnnot)){
      molecules=params$molecules; cells=params$cells; times = params$times;
      print(type_nme)
 ...
 #print(head(countdata[[1]]))
  ...
  observeEvent(input$dir, {readDir(input$dir, update=T)}
	...
	output$infPlot<-renderPlot({
	  ...
	  input$plotButton
	  infectivityPlot(params = gather_input_activity())
	})
	...
	output$distPlot <- renderPlot({
	    ...
	    input$plotButton
  	    transcriptPlot(params = gather_input_abundance())
  	  })

	output$depthPlot <- renderPlot({
	shiny::validate(need(input$toplot5 != "-" | input$toplot7 != "", ''))
	    input$plotButton
	  depthPlot(params = gather_input_depth(), "depth")
	})
	output$depthStartPlot <- renderPlot({
	shiny::validate(need(input$toplot5 != "-" | input$toplot7 != "", ''))
	  input$plotButton
	  depthPlot(params = gather_input_depth(),"depthStart")
	})
	output$depthEndPlot <- renderPlot({
	shiny::validate(need(input$toplot5 != "-" | input$toplot7 != "", ''))
	  input$plotButton
	  depthPlot(params = gather_input_depth(),"depthEnd")
